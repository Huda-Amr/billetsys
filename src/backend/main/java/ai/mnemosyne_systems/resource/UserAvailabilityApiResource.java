...
public class UserAvailabilityApiResource {
    ...
    @GET
    @Path("/user/{userId}")
    public Response getUserAvailability(@CookieParam(AuthHelper.AUTH_COOKIE) String auth,
            @PathParam("userId") Long userId) {

        User currentUser = requireAuthenticated(auth);
        ...
        boolean isOwnAvailability = currentUser.id.equals(targetUser.id);

        if (!isOwnAvailability) {

            boolean canViewOthers = AuthHelper.isSupport(currentUser) || AuthHelper.isSuperuser(currentUser);

            if (!canViewOthers) {
                return Response.status(Response.Status.FORBIDDEN).build();
            }

            Company currentCompany = Company.find("select c from Company c join c.users u where u = ?1", currentUser)
                    .firstResult();

            Company targetCompany = Company.find("select c from Company c join c.users u where u = ?1", targetUser)
                    .firstResult();

            if (currentCompany == null || targetCompany == null || !currentCompany.id.equals(targetCompany.id)) {

                return Response.status(Response.Status.FORBIDDEN).build();
            }
        }

        return Response.ok(service.getForUser(targetUser).stream().map(this::toResponse).toList()).build();
    }
    ...
    public Response getCompanyAvailability(@CookieParam(AuthHelper.AUTH_COOKIE) String auth) {
        ...
        User user = requireCompanyAvailabilityViewer(auth);
    ...
    private User requireCompanyAvailabilityViewer(String auth) {
        User user = AuthHelper.findUser(auth);
        ...
        if (!AuthHelper.isUser(user) && !AuthHelper.isSupport(user) && !AuthHelper.isSuperuser(user)) {
            throw new NotAuthorizedException(Response.status(Response.Status.UNAUTHORIZED).build());
        }
