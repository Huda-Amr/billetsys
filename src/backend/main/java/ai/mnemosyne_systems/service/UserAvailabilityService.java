...
public class UserAvailabilityService {
    ...
    private void validateCompanyAvailabilityPermission(User user) {
        ...
        if (!User.TYPE_SUPPORT.equals(type) && !User.TYPE_SUPERUSER.equals(type)) {
    ...
    private void validateNoOverlappingPersonalAvailability(User user, LocalDate startDate, LocalDate endDate,
        ...
        if (overlaps) {
            throw new BadRequestException("Personal availability period overlaps with an existing period");
        }
    }
    ...
    private String trimOrNull(String value) {
        ...
        if (trimmed.length() > 255) {
            throw new BadRequestException("Reason must not exceed 255 characters");
        }
